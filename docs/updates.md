...
- 2025-09-29 — PAT-based VM clone path: documented PAT + credential store flow; updated checklist for home-directory clone (e.g., `~/apps/innernets`) and generalized deploy `STAGING_WORKDIR`.
...
- 2025-10-04 — Explorer now runs in-process. Removed the Surfer job submit/poll path, the scheduler finalizer, and related env vars/docs. InnerNets calls Surfer only for `/api/google-search` and `/api/read-wave`. (ref: backend/src/app/agents/surfer_workflow.py, backend/src/app/clients/surfer_client.py, backend/src/app/scheduler/runner.py, docs/*surfer*)
...
- Worker dispatcher runs the in-process Explorer by default; Surfer only provides Google search and read-wave primitives.
